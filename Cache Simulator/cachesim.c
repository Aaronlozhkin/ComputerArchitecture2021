...
int Lookup(struct node** head, long int address, int replacement, int associativity, int power_Block, int size_Sets, int pre, int lines, int size_Block){
  ...
  long int tag, index, block_Offset;
  block_Offset = address%size_Block;
  long int t = (address - block_Offset)%(size_Sets*size_Block);
  index = t/size_Block;

  if (index < 0 || index > size_Sets) return 1;

  tag = address-block_Offset-(index*size_Sets*size_Block);
